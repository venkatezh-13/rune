...
#[test]
fn test_i32_add() {
    let m = single_func("add", &[ValType::I32, ValType::I32], Some(ValType::I32),
        vec![Op::LocalGet(0), Op::LocalGet(1), Op::I32Add, Op::Return]);
    let mut inst = rt().instantiate(&m).unwrap();
    assert_eq!(inst.call("add", &[Val::I32(10), Val::I32(32)]).unwrap(), Some(Val::I32(42)));
}

#[test]
fn test_i32_sub() {
    let m = single_func("sub", &[ValType::I32, ValType::I32], Some(ValType::I32),
        vec![Op::LocalGet(0), Op::LocalGet(1), Op::I32Sub, Op::Return]);
    let mut inst = rt().instantiate(&m).unwrap();
    assert_eq!(inst.call("sub", &[Val::I32(100), Val::I32(58)]).unwrap(), Some(Val::I32(42)));
}

#[test]
fn test_i32_mul() {
    let m = single_func("mul", &[ValType::I32, ValType::I32], Some(ValType::I32),
        vec![Op::LocalGet(0), Op::LocalGet(1), Op::I32Mul, Op::Return]);
    let mut inst = rt().instantiate(&m).unwrap();
    assert_eq!(inst.call("mul", &[Val::I32(6), Val::I32(7)]).unwrap(), Some(Val::I32(42)));
}

#[test]
fn test_i32_div_s() {
    let m = single_func("div", &[ValType::I32, ValType::I32], Some(ValType::I32),
        vec![Op::LocalGet(0), Op::LocalGet(1), Op::I32DivS, Op::Return]);
    let mut inst = rt().instantiate(&m).unwrap();
    assert_eq!(inst.call("div", &[Val::I32(84), Val::I32(2)]).unwrap(), Some(Val::I32(42)));
}

#[test]
fn test_i32_div_by_zero() {
    let m = single_func("divz", &[ValType::I32, ValType::I32], Some(ValType::I32),
        vec![Op::LocalGet(0), Op::LocalGet(1), Op::I32DivS, Op::Return]);
    let mut inst = rt().instantiate(&m).unwrap();
    assert_eq!(inst.call("divz", &[Val::I32(1), Val::I32(0)]).unwrap_err(), Trap::DivisionByZero);
}

#[test]
fn test_wrapping_add() {
    let m = single_func("wadd", &[ValType::I32, ValType::I32], Some(ValType::I32),
        vec![Op::LocalGet(0), Op::LocalGet(1), Op::I32Add, Op::Return]);
    let mut inst = rt().instantiate(&m).unwrap();
    assert_eq!(inst.call("wadd", &[Val::I32(i32::MAX), Val::I32(1)]).unwrap(),
               Some(Val::I32(i32::MIN))); // wrapping
}
...
#[test]
fn test_i32_and() {
    let m = single_func("and", &[ValType::I32, ValType::I32], Some(ValType::I32),
        vec![Op::LocalGet(0), Op::LocalGet(1), Op::I32And, Op::Return]);
    let mut inst = rt().instantiate(&m).unwrap();
    assert_eq!(inst.call("and", &[Val::I32(0xFF), Val::I32(0x0F)]).unwrap(), Some(Val::I32(0x0F)));
}

#[test]
fn test_i32_or() {
    let m = single_func("or", &[ValType::I32, ValType::I32], Some(ValType::I32),
        vec![Op::LocalGet(0), Op::LocalGet(1), Op::I32Or, Op::Return]);
    let mut inst = rt().instantiate(&m).unwrap();
    assert_eq!(inst.call("or", &[Val::I32(0xF0), Val::I32(0x0F)]).unwrap(), Some(Val::I32(0xFF)));
}

#[test]
fn test_i32_xor() {
    let m = single_func("xor", &[ValType::I32, ValType::I32], Some(ValType::I32),
        vec![Op::LocalGet(0), Op::LocalGet(1), Op::I32Xor, Op::Return]);
    let mut inst = rt().instantiate(&m).unwrap();
    assert_eq!(inst.call("xor", &[Val::I32(0xFF), Val::I32(0xFF)]).unwrap(), Some(Val::I32(0)));
}
...
#[test]
fn test_i32_eq() {
    let m = single_func("eq", &[ValType::I32, ValType::I32], Some(ValType::I32),
        vec![Op::LocalGet(0), Op::LocalGet(1), Op::I32Eq, Op::Return]);
    let mut inst = rt().instantiate(&m).unwrap();
    assert_eq!(inst.call("eq", &[Val::I32(5), Val::I32(5)]).unwrap(), Some(Val::I32(1)));
    assert_eq!(inst.call("eq", &[Val::I32(5), Val::I32(6)]).unwrap(), Some(Val::I32(0)));
}

#[test]
fn test_i32_lt_s() {
    let m = single_func("lt", &[ValType::I32, ValType::I32], Some(ValType::I32),
        vec![Op::LocalGet(0), Op::LocalGet(1), Op::I32LtS, Op::Return]);
    let mut inst = rt().instantiate(&m).unwrap();
    assert_eq!(inst.call("lt", &[Val::I32(-1), Val::I32(0)]).unwrap(), Some(Val::I32(1)));
    assert_eq!(inst.call("lt", &[Val::I32(1), Val::I32(0)]).unwrap(), Some(Val::I32(0)));
}
...
#[test]
fn test_i64_add() {
    let m = single_func("add64", &[ValType::I64, ValType::I64], Some(ValType::I64),
        vec![Op::LocalGet(0), Op::LocalGet(1), Op::I64Add, Op::Return]);
    let mut inst = rt().instantiate(&m).unwrap();
    let big = 1_000_000_000i64;
    assert_eq!(inst.call("add64", &[Val::I64(big), Val::I64(big)]).unwrap(),
...
#[test]
fn test_f64_sqrt() {
    let m = single_func("mysqrt", &[ValType::F64], Some(ValType::F64),
...
fn test_local_set_get() {
    ...
    m.functions.push(Function::new(
        ...
        vec![
            Op::LocalGet(0), Op::LocalGet(0), Op::I32Add,
            Op::LocalSet(1),
            Op::LocalGet(1),
            Op::Return,
        ],
    ...
    let mut inst = rt().instantiate(&m).unwrap();
    assert_eq!(inst.call("double", &[Val::I32(21)]).unwrap(), Some(Val::I32(42)));
}
...
fn test_constant_return() {
    let m = single_func("answer", &[], Some(ValType::I32),
...
fn test_select() {
    ...
    let m = single_func("sel", &[ValType::I32, ValType::I32, ValType::I32], Some(ValType::I32),
        vec![Op::LocalGet(0), Op::LocalGet(1), Op::LocalGet(2), Op::Select, Op::Return]);
    let mut inst = rt().instantiate(&m).unwrap();
    assert_eq!(inst.call("sel", &[Val::I32(1), Val::I32(2), Val::I32(1)]).unwrap(), Some(Val::I32(1)));
    assert_eq!(inst.call("sel", &[Val::I32(1), Val::I32(2), Val::I32(0)]).unwrap(), Some(Val::I32(2)));
}
...
fn test_memory_store_load() {
    ...
    let m = single_func("memtest", &[], Some(ValType::I32), vec![
        Op::I32Const(0),         // address
        Op::I32Const(99),        // value
        Op::I32Store { align: 2, offset: 0 },
        Op::I32Const(0),
        Op::I32Load { align: 2, offset: 0 },
        Op::Return,
    ]);
    let mut inst = rt().instantiate(&m).unwrap();
...
fn test_memory_oob() {
    ...
    let m = single_func("oob", &[], Some(ValType::I32), vec![
        Op::I32Const(i32::MAX),
        Op::I32Load { align: 2, offset: 0 },
        Op::Return,
    ]);
...
#[test]
fn test_memory_grow() {
    let m = single_func("grow", &[], Some(ValType::I32), vec![
...
#[test]
fn test_memory_size() {
    let m = single_func("msize", &[], Some(ValType::I32), vec![
        Op::MemorySize,
        Op::Return,
    ]);
    let mut inst = rt().instantiate(&m).unwrap();
...
fn test_data_segment() {
    let mut m = single_func("read_data", &[], Some(ValType::I32), vec![
        Op::I32Const(0),
        Op::I32Load { align: 2, offset: 0 },
        Op::Return,
    ]);
...
fn test_if_then_else() {
    ...
    let m = single_func("abs", &[ValType::I32], Some(ValType::I32), vec![
        Op::LocalGet(0),
        Op::I32Const(0),
        Op::I32LtS,
    ...
    let mut inst = rt().instantiate(&m).unwrap();
    assert_eq!(inst.call("abs", &[Val::I32(-5)]).unwrap(), Some(Val::I32(5)));
...
fn test_block_br() {
    ...
    let m = single_func("blk", &[], Some(ValType::I32), vec![
...
fn test_internal_call() {
    ...
    let mut m = Module::new();
    // func[0] = mul(a, b) = a * b
    m.functions.push(Function::new("mul", FuncType {params: vec![ValType::I32, ValType::I32], results: vec![ValType::I32]}, vec![], vec![Op::LocalGet(0), Op::LocalGet(1), Op::I32Mul, Op::Return]));
    // func[1] = square(x) = mul(x, x)
    m.functions.push(Function::new("square", FuncType {params: vec![ValType::I32], results: vec![ValType::I32]}, vec![], vec![Op::LocalGet(0), Op::LocalGet(0), Op::Call(0), Op::Return]));
    m.exports.push(("square".into(), 1));
    let mut inst = rt().instantiate(&m).unwrap();
    assert_eq!(inst.call("square", &[Val::I32(7)]).unwrap(), Some(Val::I32(49)));
}
...
fn test_host_call() {
    ...
    let mut m = Module::new();
    m.register_host("log", FuncType { params: vec![ValType::I32], results: vec![] },
        move |args| {
            log2.lock().unwrap().push(args[0].as_i32().unwrap());
            Ok(None)
        });
    m.functions.push(Function::new("run", FuncType {params: vec![], results: vec![]}, vec![], vec![
            Op::I32Const(42), Op::CallHost(0),
            Op::I32Const(7),  Op::CallHost(0),
            Op::Return,
        ]));
...
fn test_module_roundtrip() {
    ...
    assert_eq!(inst.call("add", &[Val::I32(20), Val::I32(22)]).unwrap(), Some(Val::I32(42)));
}
...
fn test_i64_extend_i32_s() {
    let m = single_func("ext", &[ValType::I32], Some(ValType::I64),
        vec![Op::LocalGet(0), Op::I64ExtendI32S, Op::Return]);
    let mut inst = rt().instantiate(&m).unwrap();
    assert_eq!(inst.call("ext", &[Val::I32(-1)]).unwrap(), Some(Val::I64(-1)));
}

#[test]
fn test_f64_promote_f32() {
    let m = single_func("prom", &[ValType::F32], Some(ValType::F64),
...
fn test_undefined_export() {
    let m = Module::new();
    let mut inst = rt().instantiate(&m).unwrap();
    assert!(matches!(inst.call("nope", &[]), Err(Trap::UndefinedExport(_))));
}
...
fn bench_fibonacci_30() {
    let mut m = Module::new();
    m.functions.push(Function::new("fib", FuncType {params: vec![ValType::I32], results: vec![ValType::I32]}, vec![], vec![
...
fn memory_stress_100_pages() {
    ...
    m.functions.push(Function::new("nop", FuncType {params: vec![], results: vec![]}, vec![], vec![Op::Return]));
...
fn test_fibonacci() {
    ...
    assert_eq!(inst.call("fib", &[Val::I32(10)]).unwrap(), Some(Val::I32(55)));
}
